...
#![doc(html_root_url = "https://docs.rs/ferogram-msgbox/0.6.4")]
...
//! with [`ferogram`](https://crates.io/crates/ferogram) instead.
//!
//! # What it does
//!
...
//!
//! - Tracks the global `pts`/`qts`/`seq`/`date` counters, plus a separate
//!   `pts` per channel.
//! - Detects gaps (an update arrives with a `pts_count` that doesn't chain
//!   onto the last known `pts`) and buffers out-of-order updates for a short
//!   window before giving up and requesting a diff.
//! - Decides when a periodic catch-up `getDifference` is due, even with no
//!   known gap (`NO_UPDATES_TIMEOUT`, 15 minutes).
//! - Classifies your own outgoing RPC responses ([`classify_own_response`])
//!   so updates piggybacked on them, e.g. `sendMessage` returning the new
//!   message, get folded into the same pts sequence.
//! - Applies `updates.getDifference` / `updates.getChannelDifference`
//!   results back into state once the caller has fetched them.
//!
...
//! ```
//!
//! On reconnect, feed [`UpdatesLike::ConnectionClosed`] in, anything sent
