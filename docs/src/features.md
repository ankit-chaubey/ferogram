...
```toml
# SQLite session only
ferogram = { version = "0.6.5", features = ["sqlite-session"] }

# LibSQL / Turso session (new in 0.2.0)
ferogram = { version = "0.6.5", features = ["libsql-session"] }

# HTML parsing (minimal, no extra deps)
ferogram = { version = "0.6.5", features = ["html"] }

# HTML parsing (spec-compliant, adds html5ever dep)
ferogram = { version = "0.6.5", features = ["html5ever"] }

# Multiple features at once
ferogram = { version = "0.6.5", features = ["sqlite-session", "html"] }
```
...
```toml
ferogram-tl-types = { version = "0.6.5", features = ["tl-api", "impl-serde"] }
```
...
```toml
ferogram-tl-types = { version = "0.6.5", features = ["tl-api", "name-for-id"] }
```
...
```toml
ferogram-tl-types = { version = "0.6.5", default-features = false, features = ["tl-api"] }
```
...
```toml
ferogram = "0.6.5"   # already includes StringSessionBackend
```
