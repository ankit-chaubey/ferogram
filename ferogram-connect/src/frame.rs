...
pub async fn recv_frame_read(
    ...
    match kind {
        ...
        FrameKind::Full { recv_seqno, .. } => {
            ...
            // Strict receive sequence validation.
            ...
            if recv_seq != expected_seq {
                let hex: String = len_buf
                    ...
                    .join(" ");
                tracing::error!(
                    "[ferogram/frame] Full transport: seqno mismatch                      (got {recv_seq}, expected {expected_seq}, call#{full_call_id},                      gap={}) first_bytes=[{hex}]",
